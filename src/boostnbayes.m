...
disp(['number of boosting rounds = ' num2str(nrounds)]);
...
for k = 1:nfolds
    ...
    p = ones(restsize,1)/restsize;
    weights = zeros(nrounds,1);
    for i = 1:nrounds
        s = sample(p',restsize);
        train = rest(:,s);
    ...
    for n = 1:fsize
        probs_spam = zeros(nrounds,1);
        probs_legit = zeros(nrounds,1);
        for i = 1:nrounds
            ...
            probs_spam(i) = prob_spam;
            prob_legit = sum(on_prob_legit(fold(:,n) == 1)) + ...
                         sum(off_prob_legit(fold(:,n) == 0)) + legit_prior;
            probs_legit(i) = prob_legit;
        end

        ind = fsize*(k-1)+n;
        if charac(ind) %spam
            if weights'*(probs_spam - probs_legit) > log(thresh) 
                ss = ss + 1;
            else
                sl = sl + 1;
                num_errs = num_errs + 1;
